...
class PublicacionController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $publicaciones = Publicacion::query()
            ...
            ->when(
                in_array($estado, ['borrador', 'publicado', 'archivado'], true),
    ...
    public function crear(): View
    {
        ...
        return view('admin.publicaciones.crear', compact('categorias'));
    }
    ...
    public function guardar(Request $request): RedirectResponse
    {
        ...
        try {
            if ($request->hasFile('imagen_portada')) {
                $rutaPortada = $request
                    ->file('imagen_portada')
                    ->store('publicaciones/portadas', 'public');
            }

            if ($request->hasFile('archivo_adjunto')) {
                $rutaAdjunto = $request
                    ->file('archivo_adjunto')
                    ->store('publicaciones/adjuntos', 'public');
            }

            Publicacion::create([
                'categoria_publicacion_id' => $datos['categoria_publicacion_id'],
                'usuario_id' => auth()->id(),
                'titulo' => $datos['titulo'],
            ...
            if ($rutaAdjunto) {
                Storage::disk('public')->delete($rutaAdjunto);
            }
    ...
    public function editar(Publicacion $publicacion): View
    {
        $publicacion->load([
            'categoria',
            'usuario',
        ...
        return view('admin.publicaciones.editar', compact(
            'publicacion',
            'categorias'
        ));
    }
    ...
    public function actualizar(
        ...
        try {
            ...
            if (
                ($portadaNueva || $request->boolean('eliminar_portada')) &&
                $portadaAnterior
            ) {
            ...
            if (
                ($adjuntoNuevo || $request->boolean('eliminar_adjunto')) &&
                $adjuntoAnterior
            ) {
                Storage::disk('public')->delete($adjuntoAnterior);
            }

            return redirect()
                ->route('admin.publicaciones.editar', $publicacion)
                ->with(
                    'success',
            ...
            if ($adjuntoNuevo) {
                Storage::disk('public')->delete($adjuntoNuevo);
            }
    ...
    private function validar(Request $request): array
    {
        return $request->validate([
            'categoria_publicacion_id' => [
                'required',
                'integer',
                Rule::exists('categorias_publicacion', 'id')
            ...
            'categoria_publicacion_id.required' => 'Selecciona una categoría.',
            'categoria_publicacion_id.exists' => 'La categoría seleccionada no es válida.',
            'titulo.required' => 'El título es obligatorio.',
            'titulo.max' => 'El título no debe superar los 200 caracteres.',
            'contenido.required' => 'El contenido es obligatorio.',
            'contenido.max' => 'El contenido es demasiado extenso.',
            ...
            'archivo_adjunto.mimes' => 'El adjunto debe ser PDF, Word, Excel, PowerPoint, TXT o ZIP.',
            'archivo_adjunto.max' => 'El adjunto no debe superar los 20 MB.',
            'fecha_vencimiento.after_or_equal' => 'La fecha de vencimiento no puede ser anterior a la publicación.',
    ...
    private function generarSlug(
        ...
        while (
            Publicacion::query()
                ->when(
                    $ignorarId,
                    fn ($query) => $query->whereKeyNot($ignorarId)
                )
                ->where('slug', $slug)
                ->exists()
        ) {
